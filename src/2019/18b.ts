...
export const toTunnel = ({
  ...
  for (let i = 0; i < grid.length; i++)
    for (let j = 0; j < grid[i].length; j++) {
      const p = { row: i, col: j };
      if (/^[a-z]$/.test(grid[i][j])) keys.set(grid[i][j], p);
      else if (/^[A-Z]$/.test(grid[i][j])) doors.set(grid[i][j], p);
      else if (grid[i][j] === '@')
...
const getNearestKeysMap = (tunnel: ITunnel) => {
  ...
  const distancesFromKeys = Array.from(keys.entries()).map(([k, p]) => [
    k,
    getDistancesToAllKeys(p, tunnel),
  ]);
  const distancesFromEntrances = Array.from(entrances.entries()).map(
    ([k, p]) => [k, getDistancesToAllKeys(p, tunnel)]
  );
  return new Map(
    distancesFromKeys.concat(distancesFromEntrances) as [
      string,
      IKeyToKeyInfo[]
    ][]
  );
...
const getKeyToEntranceMap = (
  ...
  for (const e of entrances.keys()) {
    ...
    for (const k of keytoKeyMap.get(e)!) keyToEntranceMap.set(k.toKey, e);
  }
  return keyToEntranceMap;
};

const getAllValuesExcludingKey = (
  keyToExclude: string,
  allKeys: Map<string, string>
) => {
  let values = '';
  for (const k of allKeys.keys())
    if (k !== keyToExclude) values += allKeys.get(k);
  return values;
};
...
export const lowerBound = (
  ...
  const allKeysInQuadrant = new Map(
    Array.from(entrances.keys()).map((k) => [
      k, // key of the Map
      keytoKeyMap
        .get(k)!
        .map((k1) => k1.toKey)
  ...
  const keysPerQuadrant = new Map(
    Array.from(entrances.keys()).map((k) => [k, keytoKeyMap.get(k)!.length])
  );
  const minSteps = new Map(
    Array.from(entrances.keys()).map((k) => [k, Infinity])
  );
  ...
  while (!queue.isEmpty()) {
    ...
    const keysObtainedOtherQuadrants = getAllValuesExcludingKey(
      quadrant,
      allKeysInQuadrant
    );
    const keysObtainedSet = new GenericSet(
      (k) => k,
      [...keysObtained, ...keysObtainedOtherQuadrants]
    );
    ...
    for (const key of reachableKeys) {
      ...
      if (
        !visited.has(newCacheKey) ||
        visited.get(newCacheKey)! > newTotalSteps
      ) {
        queue.enqueue({
...
export const solve = (
  ...
  while (!queue.isEmpty()) {
    ...
    const allRouteOptionsForAllRobots = combinations(
      ...
      atKeys.map((k) =>
        [{ toKey: k, steps: 0 }].concat(getReachableKeys(k, keysObtainedSet))
      )
    ...
    for (const routeOptionForAllRobots of allRouteOptionsForAllRobots) {
      ...
      const newKeysObtained = Array.from(
        new GenericSet(
          (k) => k,
          robotsMoveToKey
      ...
      const newCacheKey = getCacheKey(
        robotsMoveToKey.join(''),
        newKeysObtained
      );
      const newTotalSteps = routeOptionForAllRobots.reduce(
        (a, c) => a + c.steps,
        totalSteps
      );
      if (
...
export const run = () => {
  ...
  for (const s of sims) {
    ...
    console.log(
      timer.start(`18b - ${s.name}, ${s.keys.size} keys, ${s.doors.size} doors`)
    );
