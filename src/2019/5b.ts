...
const command = {
    '1': (program: number[]) => (start: number, modes: IMode[], _: number) => {
        ...
        return { output: NaN, next: size['1'] + start + 1};
    ...
    '5': (program: number[]) => (start: number, modes: IMode[], _: number) => {
        const op1 = program[getIndex(program)(start + 1, modes[0])];
        const op2 = program[getIndex(program)(start + 2, modes[1])];
        return { output: NaN, next: op1 !== 0 ? op2 : size['5'] + start + 1 };
    },
    ...
    '7': (program: number[]) => (start: number, modes: IMode[], _: number) => {
        ...
        return { output: NaN, next: size['7'] + start + 1};
        // console.log({op1, op2, program});
    },
    '8': (program: number[]) => (start: number, modes: IMode[], _: number) => {
        ...
        const op2 = program[getIndex(program)(start + 2, modes[1])];
        program[getIndex(program)(start + 3, modes[2])] = op1 === op2 ? 1 : 0;
        return { output: NaN, next: size['8'] + start + 1};
        // console.log({op1, op2, program});
    },
    '9': (_: number[]) => (__: number, ___: IMode[], _: number) => ({output: NaN, next: -1})
};
/* eslint-enable */
...
export const restore = (program: number[]) => (input: number) => {
    ...
    while (i < program.length) {
        ...
        const {output, next} = command[opcode](program)(i, modes, input);
        // console.log({opcode, modes, output, next, program});
        if (!isNaN(output)) {
            return { output, modes: getModes(program[i]) };
        }
        if (opcode === '9')
            return program[0];
        i = next;
