...
export const equals = (p1?: IPoint, p2?: IPoint) =>
  p1 != null && p2 != null ? p1.col === p2.col && p1.row === p2.row : false;
...
export const getNeighbors = (
  ...
  exclude == null
    ...
    : getNeighbors({ row, col }, null).filter(
        (p) => p.row !== exclude.row || p.col !== exclude.col
      );

export const makeIsValid =
  ({ grid, keys, doors }: IBaseTunnel) =>
  (visited: GenericSet<IPoint>) =>
  (p: IPoint) => {
...
export const getDistancesToAllKeys = (from: IPoint, tunnel: IBaseTunnel) => {
  ...
  while (!queue.isEmpty()) {
    ...
    const neighbors = getNeighbors(point).filter(isValid).map(addPointInfo);

    for (let i = 0; i < neighbors.length; i++) {
      const neighbor = neighbors[i];
      const newKeysObtained =
        i === neighbors.length - 1
          ? keysObtained
          : new GenericSet(keysObtained);
      const newDoorsInWay =
        i === neighbors.length - 1 ? doorsInWay : new GenericSet(doorsInWay);
      if (neighbor.door != null) newDoorsInWay.add(neighbor.door);
      if (neighbor.key != null) newKeysObtained.add(neighbor.key);
...
export const getNearestKeysMap = (tunnel: ITunnel) => {
  ...
  return new Map(
    Array.from(keys.entries()).map(([k, p]) => [
      k,
...
export const makeGetReachableKeys =
  ...
  (fromKey: string, keysObtained: GenericSet<string>) =>
    keyToKeyMap
      ...
      ?.filter((k) =>
        k.doorsInWay.subsetOf(keysObtained, (k) => k.toString().toLowerCase())
      )
...
export const solve = (keyToKeyMap: Map<string, IKeyToKeyInfo[]>) => {
  ...
  const helper = (fromKey: string, keysObtained: string): number => {
    ...
    const reachableKeys = getReachableKeys(
      fromKey,
      new GenericSet((k) => k, [...keysObtained])
    );
    ...
    for (const reachableKey of reachableKeys) {
      const steps =
        ...
        Math.min(
          totalSteps,
          helper(reachableKey.toKey, keysObtained + reachableKey.toKey)
        );
...
export const run = () => {
  ...
  for (const s of sims) {
    ...
    console.log(
      timer.start(`18 - ${s.name}, ${s.keys.size} keys, ${s.doors.size} doors`)
    );
