...
1. Ores at time `t` is equal to ores `t - 1` plus number of ore robots at `t - 1` minus all the ores used at time `t`.
   $$
   \forall_t \space\space\space\space\space\space\space \text{Ore}_t =
   \text{Ore}_{t-1} + \text{OreRobot}_{t-1} - c_{ore,ore}\dot
   \text{OreRobotBuilt}_t - c_{clay,ore}\dot\text{ClayRobotBuilt}_t
   - c_{obsidian,ore}\dot\text{ObsidianRobotBuilt}_t - c_{geode,ore}\dot\text{GeodeRobotBuilt}_t
   $$
