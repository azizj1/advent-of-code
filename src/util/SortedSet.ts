...
export class SortedSet<E> extends GenericSet<E> {
  ...
  add(e: E) {
    if (this.lazySortedArray == null) this.lazySortedArray = [];
    if (!this.has(e)) {
  ...
  delete(e: E) {
    ...
    if (hasItem)
      this.lazySortedArray.splice(
        this.lazySortedArray.findIndex((item) => this.getHash(item) === hash),
        1
      );
    return hasItem;
  }
  ...
  get sortedValues() {
    if (this.isSorted) return this.lazySortedArray;
    this.isSorted = true;
