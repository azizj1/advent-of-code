...
async function getLatestFile(dir: string, files: string[]) {
  ...
  const filesWithTime = files.map((f, i) => ({
    file: f,
    time: times[i].mtime.getTime(),
  }));
  return filesWithTime.reduce(
    (max, c) => (c.time > max.time ? c : max),
    filesWithTime[0]
  ).file;
}
...
export async function runLastChanged() {
  ...
  try {
    const p = await import(
      /* webpackChunkName: "customProblem" */ `./2020/${sanitized}.ts`
    );
    if (typeof p.test === 'function') {
      p.test();
    }
    // always execute the run function.
    p.run();
  } catch (e) {
    console.log(e);
  }
